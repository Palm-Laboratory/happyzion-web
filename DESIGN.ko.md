...
토큰은 사용 범위에 따라 세 티어로 구분합니다. 이 목록에 없는 값은 일회성(one-off)으로 인라인 적용하며 토큰으로 관리하지 않습니다.

**컴포넌트** — UI 요소 내부 간격

| 단계 | 값 | 역할 |
| --- | ---: | --- |
| xs | 8px | 아이콘↔텍스트 갭, 레이블 마이크로 간격 |
| sm | 12px | 칩·배지 간격, 소형 구분선 |
| md | 16px | 모바일 기본 수평 패딩 |
| lg | 20px | 카드 내부 패딩 |
| xl | 24px | 카드 섹션 구분, 컬럼 갭 |
| 2xl | 32px | 데스크탑 shell 좌우 패딩 |
| 3xl | 36px | 카드 세로 패딩 (tall/stacked 카드) |
...
| 단계 | 값 | 역할 |
| --- | ---: | --- |
| sm | 100px | 데스크탑 섹션 상하 패딩 |
| lg | 200px | 주요 섹션 하단 여백 |
...
- about 페이지에서 섹션 헤딩과 주요 콘텐츠 사이의 기본 간격은 60px입니다.
- 데스크탑 섹션 상하 패딩 기준은 100px이며, about 페이지 주요 섹션 하단은 200px을 사용합니다.
- 세 티어 토큰 목록에 없는 값(예: 특정 컴포넌트 전용 크기)은 인라인으로 적용하며 토큰으로 관리하지 않습니다.
