...
test("board post editor keeps inline image persistence asset based", async () => {
  ...
  assert.match(simpleEditorContents, /happyzionAssetId/, "Expected SimpleEditor image persistence to preserve uploaded asset ids.");
  assert.match(imageUploadNodeContents, /happyzionAssetId/, "Expected uploaded image insertion to read the persisted asset id.");
  assert.match(imageUploadNodeContents, /happyzionStoredPath/, "Expected uploaded image insertion to read the persisted stored path.");
