...
type RawSearchParams = { year?: string; status?: string; category?: string; page?: string };
...
export default async function EducationPage({
  ...
  const { courses, totalElements, totalPages, page: currentPage } = await getEducationCourses({
    year,
    status,
    category,
    page,
  ...
  if (courses.length === 0 && totalElements > 0 && totalPages > 0 && currentPage >= totalPages) {
...
function Pagination({
  ...
  return (
    <div className="grid grid-cols-3 items-center px-5 py-2">
      ...
      <nav className="flex items-center justify-center gap-1" aria-label="교육 과정 페이지">
...
function buildPageHref(searchParams: RawSearchParams, page: number) {
  ...
  if (searchParams.category?.trim()) params.set("category", searchParams.category.trim());
  if (page > 1) params.set("page", String(page));
  const qs = params.toString();
  return `/admin/education${qs ? `?${qs}` : ""}`;
}
