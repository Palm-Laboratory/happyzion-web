...
import {
  ...
  MinistryTitle,
} from "./revival-ui-parts";

function MinistryDetailQuote({ quote }: { quote: string }) {
  return (
    <div className="relative w-full overflow-hidden border-l-[3px] border-[#6d5898] bg-white/[0.04] px-pad-lg py-pad-xl md:px-pad-4xl">
...
function MinistryTeamPanel({
  ...
  return (
    <article className="flex flex-col gap-comp-xl bg-white/[0.04] p-pad-base md:p-pad-xl">
      <div className="flex flex-col gap-comp-base">
        <div className="flex flex-col gap-comp-xxs leading-6">
          <h3 className="type-label-lg font-suit text-[#c9a96e]">{title}</h3>
          <p className="type-title-xl font-medium text-white">{english}</p>
        </div>
        <ul className="type-body-xs flex flex-col gap-comp-sm text-white/80">
          {items.map((item) => (
      ...
      <div className="bg-white/16 h-px w-full" />

      <div className="flex flex-wrap gap-comp-md">
        {groups.map((group) => (
          <span
            key={group}
            className="type-label-md font-suit inline-flex items-center gap-comp-xxs border border-[#e3bfff]/50 bg-[#e3bfff]/15 px-comp-sm py-comp-xs tracking-[0.16em] text-[#e3bfff]"
          >
...
export default function MinistryTeamsSection() {
  ...
  const activeMinistry =
    ministryDetails[selectedTabIndex] ?? ministryDetails[0];
  const moveMinistryTab = (direction: -1 | 1) => {
    setSelectedTabIndex(
      (current) =>
        (current + direction + ministryDetails.length) % ministryDetails.length,
    );
  };
        ...
        <div className="flex w-full flex-col border border-[#5d3d8a]/15 lg:flex-row">
            ...
            {ministryTabs.map((tab, index) => {
              ...
              return (
                <button
                  ...
                  className={`flex min-h-[66px] flex-col items-start justify-center gap-comp-xxs border-b border-l-2 border-[#e3bfff]/15 px-pad-xs py-pad-xxs text-left ${
                    active
                      ? "border-l-[#c9a96e] bg-[#190b2a]"
                      : "border-l-transparent bg-white/[0.08]"
                  }`}
                  aria-pressed={active}
                >
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-layout-lg bg-[#190b2a] p-pad-lg md:p-pad-4xl">
            <div className="flex w-full min-w-0 flex-col gap-comp-sm">
              ...
              <div className="flex items-start justify-between gap-comp-xl md:gap-comp-3xl">
                ...
                <div className="hidden flex-col items-end justify-center gap-comp-xs text-center uppercase text-[#e3bfff] lg:flex">
                  <p
            ...
            <div className="flex flex-col">
              <div className="grid gap-layout-xs md:grid-cols-2">
                {activeMinistry.teams.map((team) => (
                  <MinistryTeamPanel key={team.title} {...team} />
                ))}
              </div>

              <div className="flex flex-col gap-comp-base bg-[#1a1028] px-pad-sm py-pad-xs lg:flex-row lg:items-center lg:justify-between">
                <p className="type-label-lg text-[#c9a96e]">members</p>
                <div className="type-body-xs flex flex-wrap gap-x-comp-md gap-y-comp-sm text-white/80">
                  {activeMinistry.members.map((member, index) => (
                    <span
                      key={member}
                      className={
                        index === activeMinistry.members.length - 1
                          ? "text-[#c9a96e]"
                          : ""
                      }
                    >
